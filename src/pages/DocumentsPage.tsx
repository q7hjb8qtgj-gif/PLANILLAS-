...
export function DocumentsPage() {
  ...
  return <><PageHeader title="Documentos y evidencias" description="Archivos validados y asociados a la planilla correspondiente." /><Card><div className="grid gap-4 sm:grid-cols-3"><Field label="Planilla (opcional)"><Select value={payrollId} onChange={(e) => setPayrollId(e.target.value)}><option value="">Documento general</option>{data.payrolls.map((entry) => <option key={entry.id} value={entry.id}>{entry.code}</option>)}</Select></Field><Field label="Categoría"><Select value={category} onChange={(e) => setCategory(e.target.value)}><option>Excel</option><option>PDF</option><option>Imagen</option><option>Comprobante bancario</option><option>Autorización</option><option>Contrato</option><option>Informe de obra</option><option>Otro documento</option></Select></Field><Field label="Archivo (máx. 5 MB)"><Input type="file" accept=".xlsx,.xls,.pdf,.jpg,.jpeg,.png,image/*" capture="environment" onChange={(e) => upload(e.target.files?.[0])} disabled={session?.role === 'Consulta'} /></Field></div>{message && <p className="mt-4 rounded-lg bg-blue-50 p-3 text-sm text-blue-900">{message}</p>}
